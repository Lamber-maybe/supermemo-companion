---
name: worth-learning
description: Decide what is worth learning in the AI era — a direction, a candidate article, or a single fact — and gatekeep what enters SuperMemo. Use when the user asks "what should I learn?" or "is this worth learning / importing / memorizing?", brings material they are about to import, or wants an existing collection audited.
---
...
skill produces **verdicts and reasons, never cards and never summaries**.
The user does their own reading and makes their own cards in SuperMemo;
wording help lives in `skills/reading-advisor/SKILL.md`.

## Step 0 — Learner context

Read `LEARNER.md` at the repository root if it exists. If it doesn't, and
the verdicts would genuinely change based on the answers, ask at most these
three questions (one short message, then proceed with stated assumptions if
unanswered):
...
Full essay: `README.md` § Why this exists. Close by offering to apply the
...
4. **Desire** — do they *want* it as part of themselves? Long learning runs
   on joy; a dutiful plan dies in three weeks. (This is also why the kit
   never automates the reading itself.)
...
## Altitude 2 — Material ("should I import this into SuperMemo?")

The user names or pastes a candidate: article, chapter, book, course,
video, feed. Do **not** summarize it or read it for them — skim only enough
to judge. Work through:
...
3. **Density** — will incremental reading of this yield extracts the user
   wants to own, or is it motivation, entertainment, or repetition of what
   they already know?
...
|---|---|---|
| **IMPORT** | worth incremental reading | suggested priority + one-clause why |
...
Priority suggestion: use the documented 3-point scale (≈1% / 33% / 88%,
where 0% is highest), relative to the user's core domains; when hesitating
between two values, pick the *lower* priority (the higher percentage). Most
material is not top-priority.
...
**What MEMORIZE leads to:** the user makes the cards themselves — by
extracting and clozing during incremental reading, or with **Alt+A**. If
they want wording help, point to `skills/reading-advisor/SKILL.md`. Do not
volunteer to generate the cards.
...
apply the unit cascade to each item's underlying knowledge, but report
verdicts as `KEEP` / `REFORMULATE` (state what's wrong — usually too broad,
enumeration, or no context; the rewrite itself happens in
`skills/reading-advisor/SKILL.md` § Repair) / `RETIRE` (dismiss or delete
in SuperMemo; explain once: dismissing removes it from review without
deleting content). Be aggressive: every retired item is a permanent
...
- Producing cards or summaries anywhere in this skill.
